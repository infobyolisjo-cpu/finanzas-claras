                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        <div className="flex justify-between font-medium">
                                            ...
                                            <span>{b.remaining >= 0 ? `Quedan: ${formatCurrency(b.remaining)}` : `Excedido: ${formatCurrency(Math.abs(b.remaining))}`}</span>
                                        </div>
                                         <Progress value={Math.min(b.percentage, 100)} className={`mt-2 [&>*]:${getStatusColor(b.status)}`} />
                                    </div>
                                    <p className={`text-sm font-medium ${b.status === 'exceeded' ? 'text-red-500' : 'text-muted-foreground'}`}>
